...
@Component({
      ...
      <div class="conflict-layout">
        ...
        <ng-container *ngIf="selected() as resolution; else noSelection">
          <section class="evidence surface">
            ...
            <dl><div><dt>Capacity</dt><dd>{{ resolution.evidence.capacity }}</dd></div><div><dt>Peak</dt><dd>{{ resolution.evidence.peak_concurrency }}</dd></div><div><dt>Buffer</dt><dd>{{ resolution.evidence.buffer_seconds }} sec</dd></div><div><dt>Version</dt><dd>v{{ resolution.version }}</dd></div><div><dt>Freeze</dt><dd>{{ resolution.freeze.status }}</dd></div></dl>
          ...
          <section class="decision">
            ...
            <div class="workflow surface">
              <div class="freeze" *ngIf="resolution.resolution_status === 'proposed' || resolution.resolution_status === 'pending_review'" [class.blocked]="resolution.freeze.status === 'violated'">
                <div class="freeze-head"><strong>Planning input freeze</strong><span class="freeze-state">{{ resolution.freeze.status }}</span></div>
                <p *ngIf="!resolution.freeze.violations.length">Windows, stations and satellites still match the versions captured by the scan.</p>
                <ng-container *ngIf="resolution.freeze.violations.length">
                  <p>Accept is blocked until these inputs are restored or the conflict is re-scanned. Reject remains available.</p>
                  <ul><li *ngFor="let violation of resolution.freeze.violations"><span>{{ violation.object_type }} {{ violation.object_label }}</span><em>{{ violation.field.replaceAll('_', ' ') }}</em><b>{{ violation.frozen_value }} → {{ violation.current_value }}</b></li></ul>
                </ng-container>
                <small>frozen {{ format(resolution.freeze.frozen_at) }} · checked {{ format(resolution.freeze.checked_at) }}</small>
              </div>
  ...
  styles: [`
    ...
    .freeze { margin-bottom: 14px; padding: 12px; border: 1px solid #afd0be; background: #eef5f0; } .freeze.blocked { border-color: #dcaaa5; background: #f9ecea; }
    .freeze-head { display: flex; justify-content: space-between; align-items: center; gap: 10px; } .freeze-head strong { font-size: 12px; } .freeze-state { padding: 2px 8px; border: 1px solid currentColor; border-radius: 999px; color: #205d45; font-size: 9px; font-weight: 800; text-transform: uppercase; } .freeze.blocked .freeze-state { color: #8a352f; }
    .freeze p { margin: 8px 0 0; font-size: 11px; } .freeze ul { margin: 8px 0 0; padding: 0; list-style: none; display: grid; gap: 6px; } .freeze li { display: flex; flex-wrap: wrap; gap: 8px; align-items: baseline; font-size: 11px; } .freeze li em { color: #66716d; font-size: 9px; font-style: normal; text-transform: uppercase; } .freeze li b { font-weight: 700; } .freeze small { display: block; margin-top: 8px; color: #66716d; font-size: 9px; }
