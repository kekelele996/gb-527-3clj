...
import { ConflictResolution, ConflictType, CONFLICT_TYPES, FrozenInputChange } from '../types/conflict';
...
@Component({
      ...
      <div class="conflict-layout">
        ...
        <ng-container *ngIf="selected() as resolution; else noSelection">
          <section class="evidence surface">
            ...
            <div class="freeze">
              <div class="freeze-head"><span class="eyebrow">Planning input freeze</span><app-window-status-badge [status]="resolution.freeze_status" /></div>
              <p class="freeze-note blocked" *ngIf="resolution.freeze_status === 'invalidated'">Accept is blocked: frozen planning inputs changed after the scan ({{ resolution.freeze_blocked_reason }}). The conflict stays pending review; rejection remains available.</p>
              <p class="freeze-note ok" *ngIf="resolution.freeze_status !== 'invalidated'">Windows, ground stations and satellites still match the versions frozen at scan time.</p>
              <table *ngIf="resolution.frozen_changes?.length" class="freeze-table"><thead><tr><th>Object</th><th>Field</th><th>Frozen</th><th>Current</th></tr></thead><tbody><tr *ngFor="let change of resolution.frozen_changes"><td>{{ objectLabel(change) }}</td><td>{{ change.field }}</td><td>{{ renderValue(change.before) }}</td><td>{{ renderValue(change.after) }}</td></tr></tbody></table>
              <details *ngIf="resolution.frozen_inputs as frozen"><summary>Frozen snapshot · {{ format(frozen.frozen_at) }}</summary><pre>{{ frozen | json }}</pre></details>
            </div>
          </section>
  ...
  styles: [`
    ...
    .freeze { padding: 14px 18px; border-top: 1px solid #ccd3ce; } .freeze-head { display: flex; justify-content: space-between; align-items: center; gap: 10px; } .freeze-note { margin: 10px 0 0; font-size: 12px; line-height: 1.5; } .freeze-note.blocked { color: #8a352f; } .freeze-note.ok { color: #205d45; } .freeze-table { margin-top: 10px; } .freeze details { padding: 10px 0 0; }
    .freeze-warning { margin: 0 0 10px; color: #8a352f; font-size: 12px; line-height: 1.5; }
...
export class ConflictsPage implements OnInit {
  ...
  objectLabel(change: FrozenInputChange): string { return `${change.object_type.replaceAll('_', ' ')} ${change.label}`; }
  renderValue(value: unknown): string { if (Array.isArray(value)) return value.join(', '); if (value === null || value === undefined || value === '') return '—'; return String(value); }
