...
func (repository *GroundStationRepository) GetMany(ids []uint) ([]model.GroundStation, error) {
	var stations []model.GroundStation
	if err := repository.db.Where("id IN ?", ids).Order("id ASC").Find(&stations).Error; err != nil {
		return nil, fmt.Errorf("get ground stations: %w", err)
	}
	return stations, nil
}
